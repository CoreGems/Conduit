...
4. **Phase 3 — parallel** ✅ Done. The per-name FIFO id queue + a new `deferred_results` stash on `Session` handle the SDK's serial-handler-invocation pattern: when the client returns N results in one resume, the first sets its handler's Future, the rest are stashed and consumed by their handlers when they eventually fire. Pinned by `tests/test_parallel_tools.py`. See AGENTIC_INTEGRATION.md §5.8.
5. **Phase 4 — `tool_choice` + compat parity.** Compat parity ✅ Done. `tool_choice` enforcement (force a specific tool, disable parallel) still TODO — SDK doesn't expose a direct knob; would require either MCP-layer interception or a prompt-injection workaround.
